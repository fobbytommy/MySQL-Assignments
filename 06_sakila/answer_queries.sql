...
SELECT city.city_id, city.city, customer.first_name, customer.last_name, 
customer.email, address.address FROM city
...
SELECT actor.actor_id, CONCAT_WS(' ', actor.first_name, actor.last_name) AS actor_name,
...
WHERE store.store_id = 1 AND (city.city_id = 1 OR city.city_id = 42 
OR city.city_id = 312 OR city.city_id = 459);
...
#    and special feature. Hint: You may use LIKE function in getting 
#    the 'behind the scenes' part.
SELECT film.title, film.description, film.release_year, film.rating, film.special_features
FROM film
JOIN film_actor ON film.film_id = film_actor.film_id
WHERE film.rating = 'G' AND film_actor.actor_id = 15 AND
...
CONCAT_WS(' ', first_name, last_name) AS actor_name, actor.last_update
FROM film 
JOIN film_actor ON film.film_id = film_actor.film_id
...
SELECT actor.actor_id, CONCAT_WS(' ', actor.first_name, actor.last_name) AS actor_name,
film.film_id, film.description, film.release_year, film.rating, film.special_features,
category.name AS genre FROM actor
JOIN film_actor ON actor.actor_id = film_actor.actor_id
JOIN film ON film.film_id = film_actor.film_id
JOIN film_category ON film.film_id = film_category.film_id
JOIN category ON category.category_id = film_category.category_id
WHERE CONCAT_WS(' ', actor.first_name, actor.last_name) = 'SANDRA KILMER' AND
